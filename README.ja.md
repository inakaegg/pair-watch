...
- **相手がCodex CLIチャット** — Codexはこのメッセージ機能に参加できません。そこで完了順序を記録する2つのファイル（監視役が `pair-inbox.md`、実装役が `pair-outbox.md` を書く）と、Codexの会話記録（rolloutファイル）の監査に切り替えます。Codex実装役はinboxを待つあいだターンを終えません。Codex + Codexでは、Codex監視役もoutboxを待つあいだターンを保持します。完了したメッセージの連番により、待機開始前の返信や書きかけの内容を取り違えません。

Codex側の待機の設計経緯は [design-inbox-watch](plugins/pair-watch/skills/pair-watch/references/design-inbox-watch.md)（英語）にあります。

Codex監視役はCodex実装役との組み合わせに限って利用でき、起動時にCodexを相手として明示します。相手を明示しない場合は、従来のClaudeを相手とする動作を維持します。
...
- **ベンダー横断でもCodex同士でも使える。** Claudeを相手にする場合はセッション間メッセージ機能を使います。Codexを相手にする場合は、完了連番付きのinbox／outboxを使います。Codex + Codexでは両側が反対向きのファイルを待つため、通常の受け渡しで人間が毎回確認を促す必要はありません。
- **作業手順込み。** タスクの契約書、コミット前に独立レビューで合否判定の行（`VERDICT: LGTM`＝問題なし、または `VERDICT: CHANGES REQUESTED`＝要修正）を得る関門、コミットしてよい条件が、取り決めの一部です。レビュー担当は可能なら実装役と異なる系統を使います。Claudeを利用できない場合は、実装の会話を渡さない読み取り専用のCodexを代替として使い、同じ系統であることを結果に明記します。プロジェクト自身の `AGENTS.md` に同種の規則があればそちらが優先されます。
...
2. Claude側のチャットで `/pair-watch <タスク1行> — peer: Codex CLI` と打ちます。相手がCodexだと伝えることでClaude側が監視役になります。伝えないと、ClaudeはClaudeの相手を探して待ち続けます。
...
2. 読み取り専用の監視役にするチャットで、`/pair-watch <タスク1行> — peer: Codex CLI` と打ちます。
...
- *Codex監視役がoutboxに反応しない* — 上限付きの待機が終わったか、承認が必要になっています。監視役のチャットへ「check the outbox」と入力してください。
