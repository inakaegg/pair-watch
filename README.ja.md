...
- **役割の固定。基本は2席。** 実装役が1つ、読み取り専用の監視役が1つ。2席は、文脈を共有せずに一方が他方を検証できる最小の構成です。同じ手順は、複数の実装席（ユーザーが開くか、承認を得て監視役が起動する）を1つの監視役が束ねる形にも広がります（Skillの「Multiple implementer seats」節）。ただし監視役のレビュー処理量とユーザーの確認待ちの積み上がりが実用上の上限で、数席を超える並列展開には向きません。
...
従来の2席と多席運用で、あなたの見る場所はこう変わります。

```mermaid
flowchart TB
    subgraph classic["従来: 2席 — 両方のチャットを自分で見る"]
        direction LR
        U1([あなた]) --- W1["監視役チャット"]
        U1 --- I1["実装役チャット"]
        W1 <-.->|"メッセージ · 記録監査"| I1
    end
    subgraph multi["多席: 見るのは監視役チャット1つ"]
        direction LR
        U2([あなた]) --- W2["監視役チャット<br/>（質問はここへ集まり<br/>pending listで確認）"]
        W2 <-->|"指示 · 報告"| A["席A"]
        W2 <-->|"指示 · 報告"| B["席B"]
        W2 <-->|"起動 · 復旧 · 後始末"| C["席C<br/>（tmux / script）"]
    end
```
...
向いているのは、要約ではなく検証をする第二の目が欲しい非自明な変更と、監督付きでCodexに実装させたい場面です。作業手順込みなので小さな修正には過剰です。並列作業は、1つの監視役に複数の実装席をぶら下げる形で対応できます（席数の実用上の上限はSkillの「Multiple implementer seats」節を参照）。
...
- *送ったメッセージや返信がいつまでも相手へ届かない* — 受信側で配送前の承認待ちになっています。Claude Codeはセッション間メッセージを既定で受信側ユーザーの承認にかけ（`--dangerously-skip-permissions` で起動していても出ます）、一度の承認は次のメッセージへ引き継がれません。両側の `~/.claude/settings.json` に `"crossSessionInbound": "accept"` を設定するか、`/config` の「Messages from your other sessions」をacceptにしてください。
