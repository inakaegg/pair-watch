...
- *監視役のチャットを閉じて開き直したら、席が黙ってしまった* — セッションと履歴は残りますが、メッセージの宛先はプロセスごとなので変わります。開き直した監視役は自分の宛先記録を書き直し、各席の宛先を tmux セッションから導き直して、席へ新しい宛先を伝えます。どの監視役にも届かない席は、状態を契約に書いて止まります（commit・merge・他セッションへの問い合わせはしません）。`SKILL.md` の "Watcher restart and disappearance" を参照。
