...
コーディングエージェントを2つのチャットで組ませるペアプログラミングです。片方が**実装役**、もう片方が**読み取り専用の監視役**で、監視役は相手のセッションの記録まで読んで報告を検証します。起動は片側のチャットでコマンドを1つ打つだけです。
...
チャットを2つ開き、**片方だけ**に `/pair-watch <タスク1行>` と打ちます。打たれた側が自分の役割を判定し、相手のセッションを見つけ、役割ごとの指示書（brief）を届けて、監督付きの作業ループが始まります。実装役はコードを変更します。監視役はコードに触れず、実装役の報告を実物——読み取り専用のgit操作、grep、テストログ——で確かめ、コミット前の独立レビューを取り仕切ります。人間が決めるべきことは人間へ戻します。
...
- **相手がCodex CLIチャット** — Codexはこのメッセージ機能に参加できません。そこで受け渡し用のファイル2つ（監視役が指示を書く `pair-inbox.md`、実装役が報告を書く `pair-outbox.md`）と、Codexが保存する会話記録（rolloutファイル）の監査に切り替えます。Codex実装役は監視役の返事を待つあいだターンを終えず、sleepループでinboxの更新を見張ります（この待機をinbox-watchと呼びます）。指示が届いた瞬間に動き出すので、人間が毎回「inboxを確認して」と打つ必要はありません。設計の経緯は [design-inbox-watch](plugins/pair-watch/skills/pair-watch/references/design-inbox-watch.md)（英語）にあります。
...
| [Agent teams](https://code.claude.com/docs/en/agent-teams)（Claude Codeのチーム機能。実験的、環境変数で有効化） | 先導役のセッションがチームメイトを起動する。各チームメイトは完全なセッション | 先導役。人間もチームメイトへ直接話せる | なし | 先導役が計画を承認する。会話記録の監査はしない |
...
- **作業手順込み。** タスクの契約書、コミット前に独立レビューで合否判定の行（`VERDICT: LGTM`＝問題なし、または `VERDICT: CHANGES REQUESTED`＝要修正）を得る関門、コミットしてよい条件が、取り決めの一部です。プロジェクト自身の `AGENTS.md` に同種の規則があればそちらが優先されます。
...
- **コスト。** 完全なセッション2つに加えて、関門ごとにレビュー担当のプロセスが1つ動きます。単一セッションより多く、チーム機能より少ない消費です。
...
- Claude Code 2.1系（セッション間メッセージ機能 SendMessage／ListAgents／Monitor が必要）
- Codex CLI 0.147系（rolloutファイルの配置 `~/.codex/sessions/YYYY/MM/DD/`）

セッション間メッセージ機能はClaude Codeの公式機能です（v2.1.224以降）。記録の監査は、両CLIがローカルに保存しているセッションファイルを読みます（パスはプライバシー節を参照）。
