...
- 監視役がセッションを自分で起動する場合は、リポジトリの外にも小さな管理ファイルを書きます。`~/.local/state/pair-watch/seats/<watcher-id>/<run-id>/` の下に席ごとに1ファイルで、席のラベル、プロジェクトのディレクトリ、起動の方法、セッションIDを記録します。自分の席と別のrunの席を見分けるためのものです。席を閉じたときに監視役が消します。途中で止まったrunの残りは手で消してかまいません。
