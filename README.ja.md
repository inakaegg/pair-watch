...
- **役割の固定。基本は2席。** 実装役が1つ、読み取り専用の監視役が1つ。2席は、文脈を共有せずに一方が他方を検証できる最小の構成です。同じ手順は、ユーザーが開いた複数の実装席を1つの監視役が束ねる形にも広がります（Skillの「Multiple implementer seats」節）。ただし監視役のレビュー処理量とユーザーの確認待ちの積み上がりが実用上の上限で、数席を超える並列展開には向きません。
