...
あなたにとって変わるのは、見る場所です。見るのは監視役のチャット1つだけになります。各実装役からの質問と報告はそこへ集まります。あなたの判断が要る項目は1件ずつ割り込む代わりに、確認待ちの一覧としてファイルへ溜まり、あなたは別のファイルへ回答を書きます（`_ai/tasks/<slug>/` の `PENDING.md` と `ANSWERS.md`）。チャットの流れに埋もれて探し直す手間がなくなります。起動直後の生存確認、応答が止まったセッションの検知と復旧、作業後の片付けも監視役が受け持ちます（セッションの引退には、コミットされていない作業が残っている可能性があるため、あなたの指示が必要です）。
...
flowchart TB
    ...
    subgraph multi["実装役が複数 — 見るのは監視役チャット1つ"]
        direction LR
        U2([あなた]) --- W2["監視役チャット<br/>（質問はここへ集まり<br/>判断はファイルで処理）"]
        W2 <-->|"指示 · 報告"| A["実装役A"]
        W2 <-->|"指示 · 報告"| B["実装役B"]
...
実装役の人数に決まった上限はありません。レビュー役は別プロセスとして並列に走るので、レビューの量が上限を決めるわけではありません。効いてくるのは仕事の形のほうです。実装役どうしはファイルとブランチが重ならないことが前提なので、いま独立して着手できるタスクの数がそのまま上限になります。さらに効いてくるのが、人数とともに増える2つの負担です。あなたに溜まる確認待ちと、監視役自身の文脈の膨らみです。タスクが独立したぶんだけ実装役を足してください。運用の詳細はスキルの「Multiple implementer seats」節にあります。
...
- **判断の集約。** 人間の判断が要ることは、実装役が何人いてもファイル1つに集まり、回答は別のファイル1つに書きます。
...
- 2つのセッションのどちらかが、リポジトリ内に受け渡し用のファイルを書きます。置き場所はすべてメインのcheckout側の `_ai/tasks/<slug>/` です。タスクの契約書（`TASK.md`）、あなたの判断を待つ項目の一覧（`PENDING.md`）とあなたが書く回答（`ANSWERS.md`）、そしてCodex相手の場合の `pair-inbox.md` / `pair-outbox.md` です。`_ai/` はバージョン管理の対象外にしてください（`.gitignore` へ追加）。
